...
app.post("/orders", (req, res) => {
  fs.readFile("./orders.json", "utf8", (err, ordersJson) => {
    ...
    if (!order) {
      ...
      res.status(500).send("Order by id = " + req.body.id + " already exists");
      return;
    }
  });
});
...
app.put("/orders/:id", (req, res) => {
  fs.readFile("./orders.json", "utf8", (err, ordersJson) => {
    ...
    if (orderBody && orderBody.id != req.params.id) {
      ...
      res.status(500).send("Order by id = " + orderBody.id + " already exists");
      return;
    }
    var order = orders.find((ordertmp) => ordertmp.id == req.params.id);
    if (!order) {
      ...
      fs.writeFile("./orders.json", newList, (err) => {
        if (err) {
          console.log(
            "Error writing file in PUT /orders/" + req.params.id + ": " + err
          );
          res.status(500).send("Error writing file orders.json");
        } else {
          res.status(201).send(req.body);
  ...
  fs.readFile("./orders.json", "utf8", (err, ordersJson) => {
    ...
    if (orderIndex != -1) {
      ...
      fs.writeFile("./orders.json", newList, (err) => {
        if (err) {
          console.log(
            "Error writing file in DELETE /orders/" + req.params.id + ": " + err
          );
          res.status(500).send("Error writing file orders.json");
        } else {
          res.status(204).send();
      ...
      res
        .status(500)
        .send("Order by id = " + req.params.id + " does not exists");
      return;
    }
  });
});
